# OSMesa Extended Headless Test Suite (ostest.c) Specification & Source

This document contains the complete technical design, implementation details, source code, and compilation instructions for ostest.c—an expanded, self-contained headless test program for **Mesa 1.2.8** (and compatible Mesa 1.x/2.x builds).

ostest.c uses the **OSMesa (Off-Screen Mesa)** rendering interface to perform automated, headless 3D graphics regression testing without requiring an active X11 server connection, display driver, or GPU.

---

## 1. Features & Sub-Test Suite Overview

ostest.c generates a  \times 512$ 24-bit RGB PPM image (P6 binary format) containing a 4x3 grid of 10 distinct OpenGL functional tests:

| Test ID | Feature Name | Core Functions Tested | Expected Visual Output |
| :--- | :--- | :--- | :--- |
| **Test 1** | **2D Procedural Texture Mapping** | glEnable(GL_TEXTURE_2D), glTexImage2D(), glTexParameteri(), glTexCoord2f() | A 3D rotated quad mapped with a  \times 64$ red-and-white checkerboard texture pattern. |
| **Test 2** | **Alpha Blending & Transparency** | glEnable(GL_BLEND), glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA) | Two overlapping semi-transparent polygons (green and blue) showing correct color composition. |
| **Test 3** | **GLU Quadrics Engine** | gluNewQuadric(), gluQuadricDrawStyle(), gluCylinder(), gluDisk() | A 3D cylinder capped with a top disk rendered via GLU utility routines. |
| **Test 4** | **OpenGL Display Lists** | glGenLists(), glNewList(), glEndList(), glCallList() | A complex geometric star pre-compiled into a display list and instantiated multiple times. |
| **Test 5** | **Scissor Box Clipping** | glEnable(GL_SCISSOR_TEST), glScissor() | A full-viewport polygon clipped cleanly to a restricted  \times 100$ rectangular region. |
| **Test 6** | **Z-Buffer Depth Precision** | glEnable(GL_DEPTH_TEST), glDepthFunc(GL_LESS), glClearDepth(1.0) | Intersecting 3D polygons (red and yellow) with pixel-accurate z-buffer depth occlusion. |
| **Test 7** | **Complex Polyhedra** | uxSolidIcosahedron(), uxWireDodecahedron() | A purple icosahedron and a cyan wireframe dodecahedron. |
| **Test 8** | **Utah Teapot** | uxSolidTeapot() | A flat-shaded orange Utah teapot silhouette. |
| **Test 9** | **Advanced Lighting** | glEnable(GL_LIGHTING), glShadeModel(GL_SMOOTH) | A grey sphere with smooth directional shading and specular highlights. |

---

## 2. Complete C Source Code (ostest.c)

Save the following code as ostest.c:
...
### **A. IBM AIX 1.3 PS/2 (MetaWare High C / cc)**

`sh
# Standard build command on AIX 1.3
cc -I./include -O -Hon=ansi ostest.c -L./lib -lMesaaux -lMesatk -lMesaGLU -lMesaGL -lm -o ostest
`

### **B. Generic Linux / Unix (GCC & Standard Mesa)**

`sh
# Build using standard gcc and system Mesa libraries
gcc -O2 ostest.c -lOSMesa -lGLU -lGL -lm -o ostest
`

---

## 4. Execution & Output Inspection

### **Running ostest**

`sh
./ostest my_test_result.ppm
`

### **Output Verification Checklist**

1. **File Size Check**:
   The output PPM image should measure **786,470 bytes** ( \times 512 \times 3$ bytes + 38-byte header):
   `sh
   ls -l my_test_result.ppm
   `

2. **Header Check**:
   `sh
   head -n 4 my_test_result.ppm
   `
   *Expected Output:*
   `
   P6
